...
class Ju_floor_lyWidget extends BaseWidget
{
    var $_name = 'ju_floor_ly';
	var $_ttl  = 1800;
	...
	function get_category()
	{
		...
		$parent_id = $mod_jucate->get(array(
			'conditions' => 'if_show=1 and channel=6',
		));
		$categories = array();
		if($parent_id)
		{
	...
	function get_goods($num=0,$cate_id=0)
	{
		...
		$tempalte = $mod_jutemplate->get('channel=6 AND state=1');
		$ju_list = array();
		if($tempalte)
		{
			$ju_list = $mod_ju->find(array(
				'join' => 'belong_goods',
				'conditions' => 'ju.template_id='.$tempalte['template_id'].' AND ju.status=1 AND ju.channel=6 AND ju.cate_id='.$cate_id,
