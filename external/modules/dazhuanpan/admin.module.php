...
class DazhuanpanModule extends AdminbaseModule
{
    ...
    {   
		...
		$add_sql = &db();
		$add_sql->query("insert into ecm_dazhuanpan(id, title ,zhizhen, gailv, num, time) values('','$title', '$zhizhen' ,'$gailv','$num','$time')");
    ...
    {   
	    ...
	    $time =  strtotime(date("y-m-d h:i:s",time()));//当前时间戳
		$edit_sql = &db();
		$edit_sql->query("UPDATE `ecm_dazhuanpan` SET `title` = '".$title."', `zhizhen` = $zhizhen , `time` = $time, `gailv` = $gailv, `num` = $num WHERE `id` = $id");
