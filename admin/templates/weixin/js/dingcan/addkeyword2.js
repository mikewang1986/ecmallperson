...
function zsc_show(id){
	...
	if(zsc_class!='add'){
		...
		click_id = id;
		var zsc_id = id;
		var url = "index.php?app=my_wxkeyword&act=addkeyword_ajax&kid="+zsc_id;
		$.post(url,null,function(data){
			$.each(data,function(i,v){
				...
				zsc_html = "<div class=\"edit-info\">"+
						   ...
						   "<p>"+
						   "<br />"+
						   "<font>标题：</font><input id=\"zsc_titles\" type=\"text\" style=\"width:277px\" class=\"text1\""+
...
function choseImg(){
	...
	$('.zhe').show();
	$('.imgbox a').remove();
	var url = "index.php?app=my_wxkeyword&act=allimages";
	$.post(url,null,function(data){
		$.each(data,function(i,v){
			$('.imgbox').append("<a href=\"javascript:choseImages('"+v.iid+"');\"><img src=\""+SITE_URL+"/"+v.imgurl+"\" id=\"zsc_imgid"+v.iid+"\"/><span onclick=\"delImg('"+v.iid+"')\">X</span></a>");
		});
	},'json');
}
...
function delImg(num){
	...
	var url = "index.php?app=my_wxkeyword&act=delimage";
...
function btnsave(num,count){
	...
	if(ktype==1){
		...
		if(count==0){
			var url = "index.php?app=my_wxkeyword&act=save";
			$.post(url,{kename:kname,keword:kword,kecontent:kecontent,ketype:ktype},function(data){
				alert('添加成功！');
				window.location.reload();
			});
		}else{
			var url = "index.php?app=my_wxkeyword&act=update";
			var kid = $('#zsc_iid').val();
...
function zscdel(num){
	...
	var url = "index.php?app=my_wxkeyword&act=del";
