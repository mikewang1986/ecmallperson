 ...
 *    @usage    none
 */
class Widget_storeApp extends BackendApp
{
    function index()
    {
        /* 读取已安装的挂件 */
        ...
        $this->display('widget_store.index.html');
    }
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->display('widget_store.form.html');
        }
        else
        {
    ...
    function _get_isolated_file()
    {
        /* 获取存在的文件列表 */
		$store_mod = &m('store');
		$store_ids = $store_mod->find(array(
			'conditions' => '',
			'fields' => 'store_id',			
		));
		$exist_files =array();
		...
		{
        	$exist_files  = array_merge($exist_files,$this->_get_exist_file($store_id['store_id']));
    ...
    function _get_exist_file($store_id)
    {
        $files = array();
        $file_dir = ROOT_PATH . '/data/files/store_'.$store_id.'/template';
        if (!is_dir($file_dir))
        {
        ...
        while (false !== ($item = $dir->read()))
        {
            ...
            $files[] = 'data/files/store_'.$store_id.'/template/' . $item;
        }
    ...
    {
        $file = ROOT_PATH . '/external/widgets/store/' . $name;
        switch ($type)
        {
            case 'script':
