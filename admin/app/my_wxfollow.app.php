...
class My_wxfollowApp extends BackendApp {
    ...
    function My_wxfollow() {
        ...
        if (empty($ininterface)) {
            $this->show_message(Lang::get('no_ininterface'), '请先配置', 'index.php?app=app=my_wxconfig');
            exit;
        }
    }
    ...
    function index() {
        ...
        $this->display('my_wxfollow.index.html');
    }

    function showfollow() {
        $conditions = ' AND isfollow=1 AND user_id=' . $this->_store_id;
    ...
    function save() {
        if (IS_POST) {
            $kid = $this->my_wxkeyword_mod->get_follow_id();
            $kid = empty($kid) ? 0 : $kid;
