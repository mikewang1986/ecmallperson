 ...
 *    @usage    none
 */
class JubrandApp extends BackendApp
{
    var $_jubrand_mod;
    ...
    function index()
    {
        ...
        $page['item_count']=$this->_jubrand_mod->getCount();   //获取统计数据
        /* 导入jQuery的表单验证插件 */
        ...
        $this->display('jubrand.index.html');
    }
     /**
     *    新增商品品牌
     *
    ...
    function add()
    {
        if (!IS_POST)
        {
            ...
            $this->display('jubrand.form.html');
        }
        else
        {
            $data = array();
            ...
            /* 检查名称是否已存在 */
            if (!$this->_jubrand_mod->unique(trim($data['brand_name'])))
            {
                $this->show_warning('name_exist');
                return;
            }
            if (!$brand_id = $this->_jubrand_mod->add($data))  //获取brand_id
            {
                $this->show_warning($this->_jubrand_mod->get_error());
            ...
            $logo && $this->_jubrand_mod->edit($brand_id, array('brand_logo' => $logo)); //将logo地址记下

            $this->show_message('add_brand_successed',
                'back_list',    'index.php?app=jubrand',
                'continue_add', 'index.php?app=jubrand&amp;act=add'
            );
        }
    }
    ...
    function check_brand ()
    {
        ...
        if ($this->_jubrand_mod->unique($brand_name, $brand_id)) {
    ...
    function edit()
    {
        ...
        if (!$brand_id)
        {
         ...
         if (!IS_POST)
        {
            $find_data     = $this->_jubrand_mod->find($brand_id);
            if (empty($find_data))
            {
                $this->show_warning('no_such_brand');
        ...
        else
        {
            ...
            if ($logo === false)
            {
             ...
             /* 检查名称是否已存在 */
            if (!$this->_jubrand_mod->unique(trim($data['brand_name']), $brand_id))
            {
                $this->show_warning('name_exist');
                return;
            }
            $rows=$this->_jubrand_mod->edit($brand_id, $data);
            if ($this->_jubrand_mod->has_error())
            {
                $this->show_warning($this->_jubrand_mod->get_error());
            ...
            $this->show_message('edit_brand_successed',
                'back_list',        'index.php?app=jubrand',
                'edit_again',    'index.php?app=jubrand&amp;act=edit&amp;id=' . $brand_id);
        }
    }
   ...
   function ajax_col()
   {
       ...
       if (in_array($column ,array('brand_name', 'recommended', 'sort_order', 'tag')))
       {
           ...
           {
               $brand = $this->_jubrand_mod->get_info($id);

               if(!$this->_jubrand_mod->unique($value, $id))
               {
                   echo ecm_json_encode(false);
                   return ;
               }
           }
           $this->_jubrand_mod->edit($id, $data);
           if(!$this->_jubrand_mod->has_error())
           {
               echo ecm_json_encode(true);
           }
    ...
    function drop()
    {
        ...
        $this->_jubrand_mod->drop($brand_ids);
        if ($this->_jubrand_mod->has_error())    //删除
        {
            $this->show_warning($this->_jubrand_mod->get_error());
    ...
    function update_order()
    {
        ...
        {
            $this->_jubrand_mod->edit($id, array('sort_order' => $sort_orders[$key]));
        }
    ...
    function _upload_logo($brand_id)
    {
        ...
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=jubrand&amp;act=edit&amp;id=' . $brand_id);
            return false;
        }
        /* 指定保存位置的根目录 */
    ...
    function update()
    {
        ...
        $rows=$this->_jubrand_mod->edit($brand_ids, $data);
        if ($this->_jubrand_mod->has_error())
        {
            $this->show_warning($this->_jubrand_mod->get_error());
    ...
    function pass()
    {
        ...
        $ids = explode(',', $id);
        $brands = $this->_jubrand_mod->find(db_create_in($ids, 'brand_id') . " AND if_show = 0");
        $this->_jubrand_mod->edit(db_create_in(array_keys($brands), 'brand_id'), array('if_show' => 1));
        if ($this->_jubrand_mod->has_error())
        {
            $this->show_warning($this->_jubrand_mod->get_error());
            exit;
        ...
        $this->show_message('brand_passed',
            'back_list', 'index.php?app=jubrand&wait_verify=1');
    }
    ...
    function refuse()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->display('jubrand.refuse.html');
        }
        else
        {
            ...
            foreach ($brands as $brand)
            {
                ...
                $this->_jubrand_mod->drop($brand['brand_id']);
            }
