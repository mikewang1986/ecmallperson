...
class JifenApp extends BackendApp
{
	...
	function index()
	{
		...
		foreach ($indent AS $key=>$user)
		{
			$indent[$key]['tuijian_name'] = '';
			if ($user['tuijian_id'] > 0)
			{
				$user_info = $this->_member_mod->get("user_id='".$user['tuijian_id']."'");
				$indent[$key]['tuijian_name'] = $user_info['user_name'];
			}
		}
