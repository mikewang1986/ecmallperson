...
class XwbconnectApp extends MallbaseApp
{
	...
	function callback()
	{
		foreach($this->xwb_config as $key=>$val){
			define($key, $val);
		}
		...
		if ($token) {
			...
			//echo print_r($token). '授权完成,<a href="weibolist.php">进入你的微博列表页面</a><br />';
			...
			{
				$user_name = mt_rand();
				if($user_id = $ms->user->register($this->_app.$user_name, md5($user_name.mt_rand()), $user_name.'@xwb.com'))
				{
					$data = array(
						'openid' => $openid,
						'user_id'=> $user_id,
