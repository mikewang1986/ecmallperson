...
if($MsgEvent=='SCAN')
{
	if (isset($xmlObj->EventKey)){
           ...
           $contentStr = "关注二维码场景 ".$wxch_qr_info['scene'];
...
$account = $deposit_account->get(array('conditions'=>"user_id='".$user_info[user_id]."'"));
$my_money_model = & m('my_money');
$my_money_info = $my_money_model->get("user_id=".$user_info['user_id']);
  $user['jifen'] = $my_money_info['jifen'];

 $contentStr = "余额：$account[money]\r\n积分：$user[jifen]";
...
if($Content == 'ddcx')
{
 ...
 // echo $this->send($this->ToUserName, $this->FromUserName, 'yyyyyy');
     ...
     $user_id_id = $ms->user->register($local_user, $password, time().'@qq.com',$redadd);
