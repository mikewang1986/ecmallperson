...
class SdemandApp extends MallbaseApp
{
    ...
    function index()
    {
       /* 处理cate_id */
		...
		$title = !empty($_GET['keyword'])? trim($_GET['keyword']) : '';
    ...
    function view()
    {
        ...
        $cate_ids = array();
		if(!$id) {
			$this->show_warning('no_such_article','back_list', 'index.php?app=sdemand');
			return;
		}
		...
		$member_mod = &m('member');
		$member = $member_mod->get(array('conditions'=>'user_id='.$article['user_id'], 'fields'=>'im_qq,user_name'));
		if($member) {
			$article = array_merge($article, $member);
		}
