...
class Supply_demandApp extends MemberbaseApp
{
    ...
    function add()
    {
        if (!IS_POST)
        {
			...
			$this->assign("id", 0);
            $this->assign("belong", BELONG_SDEMAND);
			$this->assign('editor_upload', $this->_build_upload(array(
                'obj' => 'EDITOR_SWFU',
                'belong' => BELONG_SDEMAND,
                'item_id' => 0,
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
			...
			$this->assign('build_editor', $this->_build_editor(array(
			 ...
			 /* 属于促销活动的附件 */
            $files_belong_sdemand= $this->_uploadedfile_mod->find(array(
                'conditions' => 'store_id = '.$this->visitor->get('user_id').' AND belong = ' . BELONG_SDEMAND . ' AND item_id =' . $id,
                'fields' => 'this.file_id, this.file_name, this.file_path',
                'order' => 'add_time DESC'
            ));
			$this->assign('files_belong_sdemand', $files_belong_sdemand);
    ...
    function drop_uploadedfile()
    {
        ...
        if ($file_id && $file['store_id'] == $this->visitor->get('user_id') && $this->_uploadedfile_mod->drop($file_id))
        {
