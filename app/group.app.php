...
/* 商品 */
class GroupApp extends StorebaseApp
{
    var $_goods_mod;
    ...
    function comments()
    {
        ...
        $this->display('group.comments.html');
    }
    ...
    function saleslog()
    {
        ...
        $this->display('group.saleslog.html');
    }
    function qa()
    {
        $goods_qa =& m('goodsqa');
    ...
    function _get_common_info($id)
    {
			...
			//取团购信息
			$ju_mod =&m('ju');
			$groupbuy = $ju_mod->get('status=1 AND goods_id='.$goods['goods_id']);
			if($groupbuy)
			{
				$groupbuy['spec_price'] = unserialize($groupbuy['spec_price']);
				foreach ($goods['_specs'] as $key => $spec)
				{
					if (!empty($groupbuy['spec_price'][$spec['spec_id']]) && $goods['_specs'][$key]['price']>$groupbuy['spec_price'][$spec['spec_id']]['price'])
					{
						$goods['_specs'][$key]['group_price'] = $groupbuy['spec_price'][$spec['spec_id']]['price'];
					}
					else
					{
						$goods['_specs'][$key]['group_price'] = 0;
					}
				}
				
				$goods['gid'] 				= $groupbuy['group_id'];
				$goods['max_per_user'] 		= $groupbuy['max_per_user'];
				$goods['max_per_user_desc'] = sprintf(LANG::get('max_per_user'),$groupbuy['max_per_user']);
			}		
