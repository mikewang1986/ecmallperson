 ...
 *    @usage    none
 */
class My_datapacketApp extends MemberbaseApp
{
	var $_goods_mod;
	var $_datapacket_mod;
	var $_store_mod;
    ...
    {
        $this->My_datapacketApp();
    }
    function My_datapacketApp()
    {
        parent::__construct();
		$this->_goods_mod = &m('goods');
		$this->_datapacket_mod = &m('datapacket');
    ...
    function index()
    {
		...
		$datapacket_goods = $this->_datapacket_mod->find(array(
            'fields'=> '',
		...
		if($datapacket_goods)
		{
			foreach($datapacket_goods as $key=>$goods){
				...
				$datapacket_goods[$key] +=$goods;
			}
		}

        $page['item_count'] = $this->_datapacket_mod->getCount();
        $this->_format_page($page);
        $this->assign('datapacket_goods', $datapacket_goods);
        /* 当前位置 */
        $this->_curlocal(LANG::get('member_center'),    'index.php?app=member',
                            LANG::get('my_datapacket'), 'index.php?app=my_datapacket',
                            LANG::get('datapacket_goods'));
        ...
        //当前用户中心菜单项
        $this->_curitem('my_datapacket');

        $this->_curmenu('datapacket_goods');
        $this->assign('page_info', $page);          //将分页信息传递给视图，用于形成分页条
        $this->_config_seo('title', Lang::get('member_center') . ' - ' . Lang::get('datapacket_goods'));
        $this->display('my_datapacket.index.html');
    }
	function store()
	{
		/* 当前位置 */
        ...
        $this->display('my_datapacket.form.html');
	}
	function create()
	{
		import('phpzip.lib');
		...
		{
			$goods_list = $this->_datapacket_mod->find(array('conditions'=>'user_id = ' . $this->visitor->get('user_id')));
			if($goods_list)
			{
    ...
    function drop()
    {
		...
		$goods_ids = explode(',', $goods_ids);
        $this->_datapacket_mod->drop($goods_ids);
		$this->show_message('drop_datapacket_goods_successed');
    }
     ...
     *    加入数据包
     */
    function _add_datapacket($goods_id)
    {
        /* 验证商品是否存在 */
		...
		/* 加入到数据包 */
		$datapacket = $this->_datapacket_mod->get(array(
        ...
        $this->send_feed('add_datapacket_goods', array(
            'user_id'   => $this->visitor->get('user_id'),
        ...
        /* 加入成功 */
        $this->json_result('', 'add_datapacket_goods_ok');
    }
    ...
    function _get_member_submenu()
    {
        $menus = array(
            array(
                'name'  => 'datapacket_goods',
                'url'   => 'index.php?app=my_datapacket',
            ),
			array(
				'name' => 'create_store_all',
				'url'  => 'index.php?app=my_datapacket&act=store',
			),
        );
        return $menus;
    }
