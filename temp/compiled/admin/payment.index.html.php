...
<div class="tdare info">
    <table width="100%" cellspacing="0" class="dataTable">
        ...
        <tr class="tatr1">
            <td class="firstCell" width="15%">名称</td>
            <td>支付方式描述</td>
            <td width="5%">启用</td>
            ...
            <a href="index.php?app=payment&amp;act=enable&amp;code=<?php echo $this->_var['payment']['code']; ?>">启用</a>
                ...
                <?php endif; ?>
				 <?php if ($this->_var['payment']['installed']): ?>
					<a href="index.php?app=payment&amp;act=config&payment_id=<?php echo $this->_var['payment']['payment_id']; ?>&amp;code=<?php echo $this->_var['payment']['code']; ?>" >配置</a> 
					<a href="javascript:drop_confirm('卸载后所有使用该支付方式的订单将无法支付，若您只是不希望让用户可以选择该支付方式，可以使用“配置”将该支付方式禁用，您确定要卸载它吗？', 'index.php?app=payment&amp;act=uninstall&payment_id=<?php echo $this->_var['payment']['payment_id']; ?>');" class="delete">卸载</a>
				<?php else: ?>
