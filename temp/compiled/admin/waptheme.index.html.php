...
function use_theme(template, style){
    ...
    {
        if (!confirm('您选择的该微商城模板与当前使用的微商城模板不一致，因此您当前的挂件设置将不能在新微商城中显示，您需要重新设置，您确定要使用该模板吗？'))
        {
            return;
        }
...
<p>
    <b>微商城模板设置</b><br />
当前您使用的微商城模板是:<?php echo $this->_var['curr_template_name']; ?>&nbsp;<?php echo $this->_var['curr_style_name']; ?>
</p>
...
?>
    <li>
        ...
        <?php else: ?>
        <input type="button" value="查看微商城" onclick="go_index()" class="formbtn" />
        <?php endif; ?>
        </div>
    </li>
